...
public:
	...
	struct type_rep
	{
        ...
        type_rep()
        {
            ...
            initial_update_bit_total = 0;
            total_update_bit_total = 0;
        }
...
public:
    ...
    void add_empty_state(uint32 state_index)
    {
        if(state_index >= _current_class->max_state_index)
            _current_class->max_state_index = state_index + 1;
    }
	...
	template<typename type> void add_public_slot(static_string slot_name, type *slot_type_and_offset, uint32 state_index)
	{
		...
		bool (*write_fn)(bit_stream &str, const type &var) = &write;
		bool (*read_fn)(bit_stream &str, type &var) = &read;
	...
	{
		uint32 offset = uint32(reinterpret_cast<uintptr_t>(slot_type_and_offset));
	...
	void dump()
	{
		...
		for(dictionary<type_rep *>::pointer p = _class_table.first(); p; ++p)
		{
            ...
            type_rep *t = *p.value();
			if(t->parent_class)
				parent_class_name = t->parent_class->name;
			printf("class %s : %s (%p)\n", t->name.c_str(), parent_class_name.c_str(), t->type);
            if(t->initial_update_count)
            {
                printf("  initial update count %d, avg initial bit size: %g\n", t->initial_update_count, float64(t->initial_update_bit_total)/float64(t->initial_update_count));
                if(t->total_update_count)
                    printf("  partial update count %d, avg partial update bit size: %g\n", t->total_update_count, float64(t->total_update_bit_total)/float64(t->total_update_count));
            }
			for(hash_table_array<indexed_string, field_rep>::pointer fp = t->fields.first(); fp; ++fp)
				dump_field(fp.value());
			dictionary<type_rep *>::pointer lookup_p = _class_table.find(t->name);
			if(!lookup_p)
                printf("  ERROR - LOOKUP FAIL for %s.\n", t->name.c_str());
		}
	}
	...
	void dump_type(type_record *type)
	{
		...
		if(type->is_numeric_t)
		{
			numeric_record *numeric_info = type->numeric_info;
			if(numeric_info->is_integral_t)
			{
				printf("%sint{%d..%u}", numeric_info->is_signed_t ? "" : "u", numeric_info->int_range_min, numeric_info->int_range_min + (numeric_info->int_range_size));
			}
			else if(numeric_info->is_float_t)
			{
	...
	bool type_convert(void *dest, type_record *dest_type, const void *source, type_record *source_type)
	{
		...
		if(source_type->is_numeric_t && dest_type->is_numeric_t)
		{
...
#define core_type_begin_class(registry, class_name, super_class_name, indexed) \
	registry.begin_class(#class_name, get_global_type_record<class_name>(), get_global_type_record<super_class_name>(), indexed);
