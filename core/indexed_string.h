...
class indexed_string
{
	...
	bool valid() { return _node->case_insensitive_string_id != 0; }
	
	bool compare(const indexed_string & the_string, bool case_sensitive = true) const
	{
	...
	class table
	{
		...
		public:
		table(page_allocator *allocator) : _block_allocator(allocator, this) { _init(); }
		~table() { _destroy(); }
        ...
        uint32 find_case_insensitive_id(const char8 *buffer, uint32 buffer_len)
        {
            node *the_node = find(buffer, buffer_len, false);
            if(the_node)
                return the_node->case_insensitive_string_id;
            else
                return 0;
        }
        uint32 get_next_case_insensitive_id()
        {
            return _current_string_id;
        }
	...
	table::node *_node;
    enum{
        max_net_indexed_string_len = 127,
        max_string_length = table::max_string_length,
    };

};

typedef indexed_string::table indexed_string_table;
