...
template <typename key_type, typename value_type> class hash_table_array
{
	...
	hash_table_array()
    {
        _table = table::get_null();
    }
    
	~hash_table_array()
    {
        while(size() != 0)
            _table->remove(size() - 1);
        table::deallocate(_table);
    }
	...
	struct table
	{
		...
		static table *allocate(uint32 num_elements)
		{
			...
			table *ret = (table *) mem_alloc(sizeof(table) + sizeof(node *) * (size * 2 - 1));
			ret->table_size = size;
			ret->count = 0;
