...
{
	public:
	small_block_allocator(page_allocator *allocator, block_reference_type *reference_pointer = 0) { _reference_pointer = reference_pointer; _allocator = allocator; _init(); }
	...
	bool _alloc_new_top_page()
	{
		...
		if(_num_memory_status_blocks_used == num_memory_status_blocks_per_block_page)
		{
			...
			_current_memory_status_block_page = (byte *) _allocator->allocate_page();
			if(!_current_memory_status_block_page)
			{
				_allocator->deallocate_page(new_page);
				return false;
			}
	...
	void _destroy()
	{
		while(_top_page)
		{
			...
			if(_is_page_aligned(_top_page->memory_status_block))
				_allocator->deallocate_page(_top_page->memory_status_block);
			page *temp = _top_page->prev;
			_allocator->deallocate_page(_top_page);
			_top_page = temp;
		}
	}
