...
template <typename type> static inline uint32 hash(const type &x)
{
	return x.hash();
}
...
static uint32 hash_buffer(const void *buffer, uint32 len)
{
...
typedef uint32 method_hash_type;

template <typename signature> static method_hash_type hash_method(signature the_method)
{
	return hash_buffer(&the_method, sizeof(the_method));
}

#define numeric_hash_overload(type) template<> static inline uint32 hash<type>(const type &x) { return uint32(x); }

typedef void *void_ptr;

template<> static inline uint32 hash<void_ptr>(const void_ptr &x) {
...
template<> static inline uint32 hash(const char *x) { uintptr_t i = (uintptr_t) x; return uint32(i); }
