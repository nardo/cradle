...
static bool PolygonContains(const Point *inVertices, int inNumVertices, const Point &inPoint)
{
    // Loop through edges
...
/// Check if circle at inCenter with radius^2 = inRadiusSq intersects with a polygon. Function returns true when it does and the intersection point is in outPoint
static bool PolygonCircleIntersect(const Point *inVertices, int inNumVertices, const Point &inCenter, Point::member_type inRadiusSq, Point &outPoint)
{
...
static void Swap(Point::member_type &f1, Point::member_type &f2)
{
...
/// Solve the equation inA * x^2 + inB * x + inC == 0 for the lowest x in [0, inUpperBound]. Returns true if there is such a solution and returns the solution in outX
static bool FindLowestRootInInterval(Point::member_type inA, Point::member_type inB, Point::member_type inC, Point::member_type inUpperBound, Point::member_type &outX)
{
...
/// Checks intersection between a polygon an moving circle at inBegin + t * inDelta with radius^2 = inA * t^2 + inB * t + inC, t in [0, 1]. Returns true when it does and returns the intersection position in outPoint and the intersection fraction (value for t) in outFraction
static bool SweptCircleEdgeVertexIntersect(const Point *inVertices, int inNumVertices, const Point &inBegin, const Point &inDelta, Point::member_type inA, Point::member_type inB, Point::member_type inC, Point &outPoint, Point::member_type &outFraction)
{
    // Loop through edges
...
static bool PolygonSweptCircleIntersect(const Point *inVertices, int inNumVertices, const Point &inBegin, const Point &inDelta, Point::member_type inRadius, Point &outPoint, Point::member_type &outFraction)
{
