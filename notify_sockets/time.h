...
public:
	...
	{
        uint64 current_time;
        journal_read_block(time_get_current,
                         journal_read(current_time); return time(current_time);)
		timeval t;
		gettimeofday(&t, NULL);
		current_time = uint64(t.tv_sec) * 1000 + t.tv_usec / 1000;
        journal_write_block(time_get_current, journal_write(current_time);)
