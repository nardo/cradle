...
public:
	...
	void process_request(const byte_buffer_ptr &the_request, byte_buffer_ptr &the_response, bool *cancelled, float *progress)
	{
		...
		read(s, the_nonce);
		read(s, remote_nonce);
		read(s, puzzle_difficulty);
		read(s, client_identity);
		uint32 solution = 0;
		...
		if(!*cancelled)
		{
			...
			logf(notify_socket, ("Client puzzle solved in %lli ms.", (time::get_current() - start).get_milliseconds()));
		}
	}
};
...
private:
	...
	class nonce_table : ref_object {
		...
		arena_allocator<8> _allocator;

		public:
		/// nonce_table constructor
		nonce_table(page_allocator *the_page_allocator) : _allocator(the_page_allocator) { reset(); }
	...
	public:
	client_puzzle_manager(random_generator &random_gen, page_allocator *zone)
	{
        ...
        _last_update_time = _last_tick_time;
	}
