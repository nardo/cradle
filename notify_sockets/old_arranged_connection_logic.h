...
void start_arranged_connection(notify_connection *conn)
{
	conn->set_connection_state(notify_connection::sending_punch_packets);
...
void request_introduction(notify_connection* conn, uint32 identity)
{
	logf(notify_socket, ("Sending Introduction Request to %s", conn->get_address().to_string().c_str()));
	packet_stream out;
	write(out, uint8(introduction_request_packet));
	write(out, identity);
...
{
	notify_connection* our_conn = find_connection(addr);
	if(!our_conn)
	{
		logf(notify_socket, ("Failed to find connection"));
		return;
	}
	
	logf(notify_socket, ("Received Introduction Request from %s", addr.to_string().c_str()));
	uint32 identity;
	read(stream, identity);
	notify_connection* remote_conn = NULL;
	for(int32 i = 0; i < _connection_list.size(); ++i)
	{
		logf(notify_socket, ("Checking connection with identity %u", _connection_list[i]->get_connection_parameters()._client_identity));
	...
	if(!remote_conn)
	{
		logf(notify_socket, ("No connection found with identity %u", identity));
		return;
	}
	
	logf(notify_socket, ("Sending Punch Request to %s", remote_conn->get_address().to_string().c_str()));
	packet_stream out;
	bool initiator = false;
	write(out, uint8(send_punch_packet));
	write(out, addr);
	write(out, initiator);
	out.send_to(_socket, remote_conn->get_address());
	
	
	logf(notify_socket, ("Sending Punch Request to %s", addr.to_string().c_str()));
	packet_stream ret;
	initiator = true;
	write(ret, uint8(send_punch_packet));
	write(ret, remote_conn->get_address());
	write(ret, initiator);
...
{
	logf(notify_socket, ("Received request to send punch packets from %s", addr.to_string().c_str()));
	
	notify_connection* our_conn = find_connection(addr);
	if(!our_conn)
	{
		logf(notify_socket, ("handle_send_punch_packet: No connection"));
		return;
	}
	...
	read(stream, remote_address);
	logf(notify_socket, ("Sending punch packets to %s", remote_address.to_string().c_str()));
	
	bool initiator;
	read(stream, initiator);
	
	ref_ptr<notify_connection> conn = new notify_connection(_random_generator.random_nonce(), _random_generator.random_integer(), _next_connection_index, !initiator);
	
	conn->set_address(remote_address);
	conn->set_net_socket(this);
	add_pending_connection(conn);
...
{
	logf(notify_socket, ("Received punch packet from %s", addr.to_string().c_str()));
	
	bool initiator;
	read(stream, initiator);
	if(initiator)
	{
		notify_connection* conn = find_pending_connection(addr);
		if(!conn)
		{
			logf(notify_socket, ("No pending connection for %s", addr.to_string().c_str()));
			return;
		}
		conn->clear_request();
...
void send_punch_packets(notify_connection *conn)
{
	packet_stream out;
	write(out, uint8(punch_packet));
	...
	for(uint32 i = 0; i < the_params._possible_addresses.size(); i++)
	{
		...
		//			logf(notify_socket, ("Sending punch packet (%s, %s) to %s",
...
void handle_punch(const address &the_address, bit_stream &stream)
{
	...
	//		logf(notify_socket, ("Received punch packet from %s - %s", the_address.toString(), BufferEncodeBase64(b)->get_buffer()));
	
	for(i = 0; i < _pending_connections.size(); i++)
	{
		conn = _pending_connections[i];
		if(conn->get_connection_state() != notify_connection::sending_punch_packets)
			continue;
	...
	connection_parameters &the_params = conn->get_connection_parameters();
	symmetric_cipher the_cipher(the_params._arranged_secret);
	if(!bit_stream_decrypt_and_check_hash(stream, notify_connection::message_signature_bytes, stream.get_next_byte_position(), &the_cipher))
		return;
	...
	//logf(notify_connection, ("shared secret (client) %s", conn->get_shared_secret()->encodeBase64()->get_buffer()));
	_random_generator.random_buffer(the_params._symmetric_key, symmetric_cipher::key_size);
	
	conn->set_address(the_address);
	logf(notify_socket, ("punch from %s matched nonces - connecting...", the_address.to_string().c_str()));
	
	conn->set_connection_state(notify_connection::awaiting_connect_response);
	conn->_connect_send_count = 0;
	conn->_connect_last_send_time = get_process_start_time();
...
void send_arranged_connect_request(notify_connection *conn)
{
	logf(notify_socket, ("Sending Arranged Connect Request"));
	packet_stream out;
	
	connection_parameters &the_params = conn->get_connection_parameters();
	
	write(out, uint8(arranged_connect_request_packet));
	write(out, conn->get_initiator_nonce());
	uint32 encrypt_pos = out.get_next_byte_position();
	...
	write(out, the_params.connect_data);
	
	symmetric_cipher shared_secret_cipher(conn->get_shared_secret());
	bit_stream_hash_and_encrypt(out, notify_connection::message_signature_bytes, inner_encrypt_pos, &shared_secret_cipher);
	
	symmetric_cipher arranged_secret_cipher(the_params._arranged_secret);
	bit_stream_hash_and_encrypt(out, notify_connection::message_signature_bytes, encrypt_pos, &arranged_secret_cipher);
...
void handle_arranged_connect_request(const address &the_address, bit_stream &stream)
{
	...
	notify_connection *old_connection = find_connection(the_address);
	...
	for(i = 0; i < _pending_connections.size(); i++)
	{
		...
		if(conn->get_connection_state() != notify_connection::sending_punch_packets || conn->is_initiator())
			continue;
	...
	if(!bit_stream_decrypt_and_check_hash(stream, notify_connection::message_signature_bytes, stream.get_next_byte_position(), &arraged_secret_cipher))
		return;
	...
	if(!bit_stream_decrypt_and_check_hash(stream, notify_connection::message_signature_bytes, decrypt_pos, &shared_secret_cipher))
		return;
	...
	logf(notify_socket, ("Received Arranged Connect Request"));
	...
	/*
	 ...
	 add_connection(conn);
	 remove_pending_connection(conn);
	 conn->set_connection_state(notify_connection::connected);
	 post_connection_event(conn, notify_connection_established_event_type, 0);
